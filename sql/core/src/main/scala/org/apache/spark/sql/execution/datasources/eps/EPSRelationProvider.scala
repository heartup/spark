...
class EPSRelationProvider extends CreatableRelationProvider
  with RelationProvider with DataSourceRegister {

  override def shortName(): String = "eps"
  ...
  override def createRelation(
    ...
    val partitionInfo = if (partitionColumn == null) {
      EPSPartitioningInfo(
        null, 0, 0, 0,
        EPSUtils.getTableLocations(jdbcOptions.url, jdbcOptions.table))
    } else {
      EPSPartitioningInfo(
        partitionColumn, lowerBound.toLong, upperBound.toLong, numPartitions.toInt,
        EPSUtils.getTableLocations(jdbcOptions.url, jdbcOptions.table))
    }
    val parts = EPSRelation.columnPartition(partitionInfo)
    EPSRelation(parts, jdbcOptions)(sqlContext.sparkSession)
  }
}
