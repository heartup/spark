...
/**
  * Data corresponding to one partition of a CDSRDD.
  */
case class CDSPartition(whereClause: String,
                        tableLocation: CDSTableLocation,
                        idx: Int) extends Partition {
  override def index: Int = idx
}

object CDSRDD extends Logging {

  def getCDSSqlSuffix(part: CDSPartition): String = {
  ...
  /**
    ...
    * schema.
    *
    * @param options - CDS options that contains url, table and other information.
    *
  ...
  def resolveTable(options: JDBCOptions,
                   firstPart: CDSPartition): StructType = {
    val url = options.url
    ...
    try {
      val statement = conn.prepareStatement(
        s"${dialect.getSchemaQuery(table)} ${getCDSSqlSuffix(firstPart)}")
      try {
  ...
  /**
    * Build and return CDSRDD from the given information.
    *
    * @param sc - Your SparkContext.
    * @param schema - The Catalyst schema of the underlying database table.
    * @param requiredColumns - The names of the columns to SELECT.
    * @param filters - The filters to include in all WHERE clauses.
    * @param parts - An array of CDSPartitions specifying partition ids and
...
/**
  * An RDD representing a table in a database accessed via CDS.  Both the
  * driver code and the workers must be able to access the database; the driver
  * needs to fetch the schema while the workers need to fetch the data.
  */
private[jdbc] class CDSRDD(
                             sc: SparkContext,
  ...
  filters
    .flatMap(CDSRDD.compileFilter(_, JdbcDialects.get(url)))
    .map(p => s"($p)").mkString(" AND ")

  /**
    * A WHERE clause representing both `filters`, if any, and the current partition.
    */
  private def getWhereClause(part: CDSPartition): String = {
  ...
  /**
    * Runs the SQL query against the CDS driver.
    *
    */
  override def compute(thePart: Partition, context: TaskContext): Iterator[InternalRow] = {
    ...
    val part = thePart.asInstanceOf[CDSPartition]
    conn = getConnection()
    val dialect = JdbcDialects.get(url)
    import scala.collection.JavaConverters._
    dialect.beforeFetch(conn, options.asConnectionProperties.asScala.toMap)

    // H2's CDS driver does not support the setSchema() method.  We pass a
    // fully-qualified table name in the SELECT statement.  I don't know how to
    // talk about a table in a completely portable way.

    val myWhereClause = getWhereClause(part)

    val cdsSuffix = CDSRDD.getCDSSqlSuffix(part)
