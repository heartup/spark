...
class CdsRelationProvider extends CreatableRelationProvider
  ...
  override def createRelation(
    ...
    val partitionInfo = if (partitionColumn == null) {
      CDSPartitioningInfo(
        null, 0, 0, 0,
        CdsUtils.getTableLocations(jdbcOptions.url, jdbcOptions.table))
    } else {
      CDSPartitioningInfo(
        partitionColumn, lowerBound.toLong, upperBound.toLong, numPartitions.toInt,
        CdsUtils.getTableLocations(jdbcOptions.url, jdbcOptions.table))
    }
    val parts = CDSRelation.columnPartition(partitionInfo)
    CDSRelation(parts, jdbcOptions)(sqlContext.sparkSession)
  }
