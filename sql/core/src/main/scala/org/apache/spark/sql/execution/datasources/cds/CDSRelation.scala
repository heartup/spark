...
package org.apache.spark.sql.execution.datasources.cds
...
private[sql] case class CDSTableLocation(tablePart: String,
                                         dbPart: String)

/**
  * Instructions on how to partition the table among workers.
  */
private[sql] case class CDSPartitioningInfo(
                                              column: String,
                                              lowerBound: Long,
                                              upperBound: Long,
                                              numPartitions: Int,
                                              tableLocations: Array[CDSTableLocation])

private[sql] object CDSRelation extends Logging {
  /**
    ...
    */
  def columnPartition(partitioning: CDSPartitioningInfo): Array[Partition] = {
...
private[sql] case class CDSRelation(parts: Array[Partition],
  ...
  override val schema: StructType = CDSRDD.resolveTable(jdbcOptions,
                                        parts(0).asInstanceOf[CDSPartition])

  // Check if CDSRDD.compileFilter can accept input filters
  override def unhandledFilters(filters: Array[Filter]): Array[Filter] = {
    filters.filter(CDSRDD.compileFilter(_, JdbcDialects.get(jdbcOptions.url)).isEmpty)
  }
  ...
  override def toString: String = {
    ...
    s"CDSRelation(${jdbcOptions.table})" + partitioningInfo
  }
}
